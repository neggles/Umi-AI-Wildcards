...
class TagLoader:
    ...
    def __init__(self, options):
        ...
        self.txt_basename_to_path = {
            os.path.basename(file).lower().split(".")[0]: file for file in self.all_txt_files
        }
        self.yaml_basename_to_path = {
            os.path.basename(file).lower().split(".")[0]: file for file in self.all_yaml_files
        }
            ...
            with open(txt_file_path, encoding="utf8") as file:
                ...
                if self.ignore_paths:
                    if self.loaded_tags.get(basename):
                        print(
                            f'UmiAI: Warning: Duplicate filename "{basename}" in {txt_file_path}. Only the last one will be used.'
                        )
            ...
            with open(yaml_file_path, encoding="utf8") as file:
                ...
                try:
                    ...
                    if self.ignore_paths:
                        if self.loaded_tags.get(basename):
                            print(
                                f'UmiAI: Warning: Duplicate filename "{basename}" in {yaml_file_path}. Only the last one will be used.'
                            )
                        self.loaded_tags[basename] = self.loaded_tags[key]
                except yaml.YAMLError as exc:
                    print(exc)
...
class TagSelector:
    ...
    def select(self, tag, groups=None):
        ...
        if self.previously_selected_tags.get(tag) == 50000:
            self.previously_selected_tags[tag] += 1
            print(
                f'Processed more than 50000 hits on "{tag}". This probaly is a reference loop. Inspect your tags and remove any loops.'
            )
        return False
...
class DynamicPromptReplacer:
    ...
    def replace_combinations(self, match):
        ...
        weights = [(100 - summed) / zero_weights if x == 0 else x for x in weights]
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        self.is_txt2img = is_img2img is False

        dtrue = gr.Checkbox(value=True, visible=False)
        dfalse = gr.Checkbox(value=False, visible=False)
        with gr.Accordion(
            "UmiAI Wildcards", open=False, elem_id="umiai_" + ("img2img" if is_img2img else "txt2img")
        ):
            with gr.Row():
                enabled = gr.Checkbox(
                    False,
                    label="Enable UmiAI Wildcards",
                    elem_classes=["umiai-enabled"],
                    elem_id="umiai_enabled",
                )
            with gr.Group(visible=False) as accordion:
                with gr.Tab("Settings"):
                    with gr.Row():
                        shared_seed = gr.Checkbox(
                            False,
                            label="Static wildcards",
                            elem_id="umiai_static_wildcards",
                            tooltip="Always picks the same random/wildcard options when using a static seed.",
                        )
                        same_seed = gr.Checkbox(
                            False,
                            label="Same prompt in batch",
                            elem_id="umiai_same_prompt_in_batch",
                            tooltip="Same prompt will be used for all generated images in a batch.",
                        )
                    with gr.Row():
                        cache_files = gr.Checkbox(
                            True,
                            label="Cache tag files",
                            elem_id="umiai_cache_files",
        ...
        self.infotext_fields = [
            (enabled, lambda d: gr.Checkbox.update(value="UmiAI Enabled" in d)),
            (accordion, lambda d: gr.Accordion.update(value="UmiAI Enabled" in d)),
            (shared_seed, lambda d: gr.Checkbox.update(value="UmiAI Static wildcards" in d)),
            (same_seed, lambda d: gr.Checkbox.update(value="UmiAI Same prompt in batch" in d)),
            (negative_prompt, lambda d: gr.Checkbox.update(value="UmiAI negative keywords" in d)),
        ]
    ...
    def process(
        self, p, enabled, verbose, cache_files, ignore_folders, same_seed, negative_prompt, shared_seed, *args
    ):
        if not enabled:
            return
        ...
        for cur_count in range(p.n_iter):  # Batch count
            for cur_batch in range(p.batch_size):  # Batch Size
                ...
                if debug:
                    print(f'{"Batch #"+str(cur_count) if same_seed else "Prompt #"+str(index):=^30}')
                ...
                if negative_prompt and hasattr(
                    p, "all_negative_prompts"
                ):  # hasattr to fix crash on old webui versions
                ...
                if same_seed:
                    for index, _ in enumerate(p.all_prompts):
                        p.all_prompts[index] = prompt
                    break
        ...
        if original_prompt != p.all_prompts[0]:
            ...
            if verbose:
                p.extra_generation_params["File includes"] = "|".join(TagLoader.files)
